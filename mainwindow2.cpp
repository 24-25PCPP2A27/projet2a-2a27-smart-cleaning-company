...
void MainWindow2::on_pushButton_back_clicked() {
    ...
    window1->show();
    this->close();                 // Close the second window
}
...
void MainWindow2::afficher()
{
    Produit currentProduit;
    ui->tableWidget->clearContents();  // Clear any previous data
    ui->tableWidget->setRowCount(0);   // Reset row count to 0

    // Get the model containing the products
    QSqlQueryModel* model = currentProduit.afficher();

    if (!model) {
        qDebug() << "No data to display or error fetching data.";
        return;  // Return early if no model is returned
    }
    ...
    // Set horizontal header labels based on your table columns
...
void MainWindow2::on_updateButton_clicked()
{
     Produit currentProduit;
...
void MainWindow2::on_exporterButton_clicked() {
...
void MainWindow2::on_sortByIdButton_clicked()
{
...
void MainWindow2::on_exportToCSVButton_clicked() {
    // Step 1: Prompt the user to select a file path
...
void MainWindow2::on_checkStockButton_clicked() {
...
void MainWindow2::initSerialPort() {
    serial = new QSerialPort(this);
