...
lacommande::lacommande(QWidget *parent)
    : QMainWindow(parent),
      ...
      arduinoo(new Arduinoo) // Initialisation d'Arduino
{
    ui->setupUi(this);

    // Tentative de connexion à l'Arduino
    if (arduinoo->connectToArduino() != 0) {
        QMessageBox::critical(this, "Erreur", "Impossible de se connecter à l'Arduino !");
    } else {
        connect(arduinoo->getSerial(), &QSerialPort::readyRead, this, &lacommande::onArduinoDataReceived);
...
{
    if (arduinoo->getSerial()->isOpen()) {
        arduinoo->disconnectFromArduino();
    }
    delete arduino;
    delete ui;
}
...
void lacommande::sendCommandesParMotToArduino()
{
    ...
    if (searchWord.isEmpty()) {
        QMessageBox::warning(this, "Recherche vide", "Veuillez entrer la statue à rechercher.");
        return;
    }
    ...
    if (query.exec()) {
        if (query.next()) {
            ...
            if (arduinoo->sendDataToArduino(message.toUtf8()) == 0) {
...
{
    QByteArray data = arduinoo->receiveDataFromArduino();
    if (!data.isEmpty()) {
...
{
    QString cheminRapport = "C:/Users/hibaz/Desktop/INTEGRATIONN/rapport/rapport.pdf";
...
void lacommande::on_pushButton_employe_clicked() {
    MainWindow *window1 = new MainWindow(this);
    window1->show();
    this->hide();
}

void lacommande::on_pushButton_navigate_clicked() {
...
void lacommande::on_serviceButton_clicked()
{
